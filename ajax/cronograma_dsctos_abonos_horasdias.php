...
require_once "../modelos/Cronograma_Dsctos_Abonos_Horasdias.php";

$cronograma_horasextras=new Cronograma_Dsctos_Abonos_Horasdias();
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($id_ano)){
			$rspta=$cronograma_horasextras->insertar($id_ano	,$_POST["des_fec_pag"], $_POST["fec_pag"]);
			echo $rspta ? "Cronograma registrado" : "No se pudieron registrar todos los datos del cronograma";
		}
		else {
			

			$rspta=$cronograma_horasextras->editar($id_ano, $_POST["des_fec_pag"],$_POST["fec_pag"], $_POST["desde"],$_POST["hasta"] );
	...
	case 'anular':
		$rspta=$cronograma_horasextras->anular($id_ano);
 		echo $rspta ? "Cronograma anulado" : "Cronograma no se puede anular";
	break;

	case 'mostrar':
		$rspta=$cronograma_horasextras->mostrar($id_ano);
	...
	case 'listarDetalle':
		...
		$rspta = $cronograma_horasextras->listarDetalle($id);
		$total=0;
		$cont=0;
	...
	case 'listar':
		$rspta=$cronograma_horasextras->listar();
