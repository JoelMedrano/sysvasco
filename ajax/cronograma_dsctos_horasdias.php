...
require_once "../modelos/Cronograma_Dsctos_Horasdias.php";

$cronograma_dsctos_horasdias=new Cronograma_Dsctos_Horasdias();
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($id_ano)){
			$rspta=$cronograma_dsctos_horasdias->insertar($id_ano	,$_POST["des_fec_pag"], $_POST["fec_pag"]);
			echo $rspta ? "Cronograma registrado" : "No se pudieron registrar todos los datos del cronograma";
		}
		else {
			

			$rspta=$cronograma_dsctos_horasdias->editar($id_ano, $_POST["des_fec_pag"],$_POST["fec_pag"], $_POST["desde"],$_POST["hasta"] );
	...
	case 'anular':
		$rspta=$cronograma_dsctos_horasdias->anular($id_ano);
 		echo $rspta ? "Cronograma anulado" : "Cronograma no se puede anular";
	break;

	case 'mostrar':
		$rspta=$cronograma_dsctos_horasdias->mostrar($id_ano);
	...
	case 'listarDetalle':
		...
		$rspta = $cronograma_dsctos_horasdias->listarDetalle($id);
		$total=0;
		$cont=0;
	...
	case 'listar':
		$rspta=$cronograma_dsctos_horasdias->listar();
