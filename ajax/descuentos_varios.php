<?php
require_once "../modelos/Descuentos_Varios.php";
session_start();



$descuentos_varios=new Descuentos_Varios();
...
$id_des_var=isset($_POST["id_des_var"])? limpiarCadena($_POST["id_des_var"]):"";
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		if (empty($id_des_var)){
			$rspta=$descuentos_varios->insertar(			$id_trab,	
													$fec_suc,
													$detalle,
													$num_cuotas,
													$modalidad,
			...
			$rspta=$descuentos_varios->editar(              	
													$id_des_var,
													$id_trab,	
													$fec_suc,
													$detalle,
	...
	case 'desactivar':
		$rspta=$descuentos_varios->desactivar(            $id_des_var,
												  $fec_reg,
												  $usu_reg,
												  $pc_reg
												  );
	...
	case 'activar':
		...
		$id='0';
		$id=$descuentos_varios->obtenerIdAprobador($usu_reg);

		$rspta=$descuentos_varios->activar(			   $id_des_var,
											   $fec_reg,
											   $usu_reg,
											   $pc_reg);
	...
	case 'mostrar':
		$rspta=$descuentos_varios->mostrar($id_des_var);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;

	case 'listar':
		$rspta=$descuentos_varios->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
			...
			$data[]=array(
				"0"=>$reg->fec_suc,
 				...
 				"5"=>$reg->des_modalidad,
 				"6"=>$reg->des_tip_dscto,
 				"7"=>($reg->est_des_var)?'<span class="label bg-green">Cancelado</span>':
 				...
 				"9"=>($reg->est_reg)?'<button class="btn btn-warning" onclick="mostrar('.$reg->id_des_var.')"><i class="fa fa-pencil"></i></button>':
 					'<button class="btn btn-warning" onclick="mostrar('.$reg->id_des_var.')"><i class="fa fa-pencil"></i></button>',
 				"10"=>($reg->est_reg)?
 					' <button class="btn btn-danger" onclick="desactivar('.$reg->id_des_var.')"><i class="fa fa-close"></i></button>':
 					' <button class="btn btn-primary" onclick="activar('.$reg->id_des_var.')"><i class="fa fa-check"></i></button>'
 				);
 		}
 		$results = array(
