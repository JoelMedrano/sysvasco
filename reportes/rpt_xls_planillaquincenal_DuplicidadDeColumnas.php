...
LEFT JOIN tabla_maestra_detalle AS tfop ON
  ...
  LEFT JOIN 
    ...
    WHERE hep.id_fec_abono='".$id_seg_quin."'
    AND hep.abonar='1') AS hep 
...
$sql=mysql_query("SELECT
   SUM(HOUR(dat.tot_cant_horas))
+ SUM(MINUTE(dat.tot_cant_horas))/60 AS tot_cant_horas, 
 SUM(dat.tot_cant_dias)AS tot_cant_dias, 
 SUM(HOUR(dat.tot_cant_horas_al25))
+ SUM(MINUTE(dat.tot_cant_horas_al25))/60 AS tot_cant_horas_al25, 
SUM(HOUR(dat.tot_cant_horas_al35))
+ SUM(MINUTE(dat.tot_cant_horas_al35))/60 AS tot_cant_horas_al35,  
SUM(HOUR(dat.tot_cant_horas_dom))
+ SUM(MINUTE(dat.tot_cant_horas_dom))/60 AS tot_cant_horas_dom,  
 SUM(HOUR(dat.tot_cant_horas_fer))
+ SUM(MINUTE(dat.tot_cant_horas_fer))/60 AS tot_cant_horas_fer,
( SUM(HOUR(dat.tot_cant_horas_al25))
+ SUM(MINUTE(dat.tot_cant_horas_al25))/60)
+
(SUM(HOUR(dat.tot_cant_horas_al35))
+ SUM(MINUTE(dat.tot_cant_horas_al35))/60
)
+
(
SUM(HOUR(dat.tot_cant_horas_dom))
+ SUM(MINUTE(dat.tot_cant_horas_dom))/60
)
+
(SUM(HOUR(dat.tot_cant_horas_fer))
+ SUM(MINUTE(dat.tot_cant_horas_fer))/60
)
AS tot_horas_extras
FROM tabla_vacia AS res
LEFT JOIN 
(SELECT 
'".$id_seg_quin."' AS id_quin, 
 IF(
    ADDTIME(
      CASE
        WHEN hpp_reg.cant_horas = '' 
...
LEFT JOIN tabla_maestra_detalle AS tfop ON
  ...
  LEFT JOIN 
    ...
    ON DAY(hep.fecha) = r_ext.dia 
WHERE  tr.est_reg = '1'
GROUP BY tr.id_trab ) AS dat ON
dat.id_quin='".$id_seg_quin."'  
...
while($res=mysql_fetch_array($sql)){    
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue("BD$fila", utf8_encode($res["tot_cant_horas_dom"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("BE$fila", utf8_encode($res["tot_cant_horas_fer"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("BG$fila", utf8_encode($res["tot_horas_extras"]));
...
$sql=mysql_query("SELECT DISTINCT 
  ...
  IF(
    DATE_FORMAT(tr.fec_nac_trab, '%d/%m/%Y') = '00/00/0000',
    '',
    DATE_FORMAT(tr.fec_nac_trab, '%d/%m/%Y')
  ) AS fec_nac_trab,
  IF(
    DATE_FORMAT(tr.fec_ing_trab, '%d/%m/%Y') = '00/00/0000',
    '',
    DATE_FORMAT(tr.fec_ing_trab, '%d/%m/%Y')
  ) AS fec_ing_trab,
  IF(
    DATE_FORMAT(tr.fec_sal_trab, '%d/%m/%Y') = '00/00/0000',
    '',
  ...
  (
    CASE
      ...
      AND tr.fec_ing_trab NOT BETWEEN cp.desde 
      AND cp.hasta 
      THEN (
      ...
      AND tr.fec_sal_trab < cp.desde 
      THEN ('0') 
      WHEN tr.fec_ing_trab BETWEEN cp.desde 
      AND cp.hasta 
      THEN (
        15 - (
          DATEDIFF(tr.fec_ing_trab, cp.desde)
        )
      ) - IFNULL(vac.dias, '0') - IFNULL(dme.dias, '0') - IFNULL(sub.dias, '0') - IFNULL(lco.dias, '0') - IFNULL(lsi.dias, '0') - IFNULL(het.cant_dscto_endias, '') 
      ELSE '0' 
    END * 8
  ...
  ROUND(
    ...
    ) - het.dscto_dom_hsxdias_semanal - IF(
      ROUND(
        (
          (tr.sueldo_trab / 240) * (
            TIME_TO_SEC(het.cant_dscto_enhoras) / 3600
  ...
  /*------------------------------------NUEVA FILA------------------------------------*/
  ROUND(IFNULL(dee.monto, '0.00'), 2) AS total_dscto_varios,
  ROUND(
    IFNULL(pd.bono_des_trab, '0.00'),
    2
  ) AS bonodestajo,
  ...
  CASE
    ...
    ELSE REPLACE(
      ROUND(
        ...
        + IFNULL(aee.monto, '0.00')
        /*AGREGAR LOS ABONOS  EN EFECTIVO*/
        ,
        0
      ),
      '-',
...
FROM
  ...
  LEFT JOIN 
    ...
    WHERE est_reg = '1' 
      AND id_quin = '".$id_seg_quin."') AS rqc 
  ...
  LEFT JOIN 
    (SELECT 
      tr.id_trab,
      het.tot_cant_horas AS cant_dscto_enhoras,
      het.tot_cant_dias AS cant_dscto_endias,
      het.tot_cant_horas_al25 AS cant_abono_horas_al25,
      het.tot_cant_horas_al35 AS cant_abono_horas_al35,
      het.tot_cant_horas_dom AS cant_abono_horas_dom,
      het.tot_cant_horas_fer AS cant_abono_horas_fer,
      CASE
        WHEN het.tot_cant_dias = '1' 
        THEN ROUND(
          (tr.sueldo_trab / 30) - (((tr.sueldo_trab / 30) / 48) * 40),
          2
        ) 
        WHEN het.tot_cant_dias = '2' 
        THEN ROUND(
          (tr.sueldo_trab / 30) - (((tr.sueldo_trab / 30) / 48) * 32),
          2
        ) 
        WHEN het.tot_cant_dias = '3' 
        THEN ROUND(
          (tr.sueldo_trab / 30) - (((tr.sueldo_trab / 30) / 48) * 24),
          2
        ) 
        WHEN het.tot_cant_dias = '4' 
        THEN ROUND(
          (tr.sueldo_trab / 30) - (((tr.sueldo_trab / 30) / 48) * 16),
          2
        ) 
        WHEN het.tot_cant_dias = '5' 
        THEN ROUND(
          (tr.sueldo_trab / 30) - (((tr.sueldo_trab / 30) / 48) * 8),
          2
        ) 
        ...
        FROM
          ...
          LEFT JOIN 
            ...
            FROM
              horas_extras_personal hep 
            WHERE hep.abonar = '1' 
              AND hep.id_fec_abono = '".$id_seg_quin."') AS hep 
            ON tr.id_trab = hep.id_trab 
            AND hep.fecha BETWEEN ch.desde 
            AND ch.hasta 
          LEFT JOIN 
          ...
          LEFT JOIN 
            ...
            ORDER BY MONTH(fe_ext.fecha) ASC,
              DAY(fe_ext.fecha) ASC) AS r_ext 
            ON DAY(hep.fecha) = r_ext.dia 
        GROUP BY tr.id_trab) AS het 
        ON tr.id_trab = het.id_trab) AS het 
  ...
  LEFT JOIN 
    (SELECT 
      ...
      CASE
        WHEN pre.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN pre.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN pre.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        WHEN pre.fec_des4 = '".$id_seg_quin."' 
        THEN mon_des4 
        WHEN pre.fec_des5 = '".$id_seg_quin."' 
        THEN mon_des5 
        WHEN pre.fec_des6 = '".$id_seg_quin."' 
        THEN mon_des6 
        WHEN pre.fec_des7 = '".$id_seg_quin."' 
        THEN mon_des7 
        WHEN pre.fec_des8 = '".$id_seg_quin."' 
        THEN mon_des8 
        WHEN pre.fec_des9 = '".$id_seg_quin."' 
        THEN mon_des9 
        WHEN pre.fec_des10 = '".$id_seg_quin."' 
        THEN mon_des10 
        WHEN pre.fec_des11 = '".$id_seg_quin."' 
        THEN mon_des11 
        WHEN pre.fec_des12 = '".$id_seg_quin."' 
        THEN mon_des12 
        WHEN pre.fec_des13 = '".$id_seg_quin."' 
        THEN mon_des13 
        WHEN pre.fec_des14 = '".$id_seg_quin."' 
        THEN mon_des14 
        WHEN pre.fec_des15 = '".$id_seg_quin."' 
        THEN mon_des15 
        WHEN pre.fec_des16 = '".$id_seg_quin."' 
        THEN mon_des16 
        WHEN pre.fec_des17 = '".$id_seg_quin."' 
        THEN mon_des17 
        WHEN pre.fec_des18 = '".$id_seg_quin."' 
        THEN mon_des18 
        WHEN pre.fec_des19 = '".$id_seg_quin."' 
        THEN mon_des19 
        WHEN pre.fec_des20 = '".$id_seg_quin."' 
        THEN mon_des20 
        WHEN pre.fec_des21 = '".$id_seg_quin."' 
        THEN mon_des21 
        WHEN pre.fec_des22 = '".$id_seg_quin."' 
        THEN mon_des22 
        WHEN pre.fec_des23 = '".$id_seg_quin."' 
        THEN mon_des23 
        WHEN pre.fec_des24 = '".$id_seg_quin."' 
        THEN mon_des24 
        WHEN pre.fec_des25 = '".$id_seg_quin."' 
        THEN mon_des25 
        WHEN pre.fec_des26 = '".$id_seg_quin."' 
        THEN mon_des26 
        WHEN pre.fec_des27 = '".$id_seg_quin."' 
        THEN mon_des27 
        WHEN pre.fec_des28 = '".$id_seg_quin."' 
        THEN mon_des28 
        WHEN pre.fec_des29 = '".$id_seg_quin."' 
        THEN mon_des29 
        WHEN pre.fec_des30 = '".$id_seg_quin."' 
        THEN mon_des30 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      did.id_trab,
      CASE
        WHEN did.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN did.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN did.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      dv.id_trab,
      CASE
        WHEN dv.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN dv.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN dv.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      dm.id_trab,
      CASE
        WHEN dm.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN dm.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN dm.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      aa.id_trab,
      CASE
        WHEN aa.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN aa.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN aa.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      dee.id_trab,
      CASE
        WHEN dee.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN dee.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN dee.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
      descuentos_en_efectivo AS dee 
    WHERE est_reg = '1') AS dee 
    ON dee.id_trab = tr.id_trab 
  LEFT JOIN 
    (SELECT 
      aee.id_trab,
      CASE
        WHEN aee.fec_des1 = '".$id_seg_quin."' 
        THEN mon_des1 
        WHEN aee.fec_des2 = '".$id_seg_quin."' 
        THEN mon_des2 
        WHEN aee.fec_des3 = '".$id_seg_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
     abonos_en_efectivo AS aee 
    WHERE est_reg = '1') AS aee 
    ON aee.id_trab = tr.id_trab 
  LEFT JOIN 
    (SELECT DISTINCT 
      vc.id_trab,
      (
        IFNULL(vc2.monto, 0) + IFNULL(vc1.monto, 0)
      ) AS monto,
      (
        IFNULL(vc2.monto_a_pagar, 0) + IFNULL(vc1.monto_a_pagar, 0)
      ) AS monto_a_pagar,
      (
        IFNULL(vc2.dias, 0) + IFNULL(vc1.dias, 0)
      ) AS dias,
      CONCAT(
        IFNULL(vc2.fechas, ''),
        ' ',
    ...
    FROM
      ...
      LEFT JOIN 
        (SELECT 
          vc.id_trab,
          IF(vc.id_cp = '".$id_seg_quin."', 'SI', '0.00') AS monto,
          CASE
            WHEN tr.id_pag_vac_cts = '1' 
            THEN ROUND(
              (tr.sueldo_trab / 30) * pd.dias_vc + 0.0000000001,
              2
            ) 
            WHEN tr.id_pag_vac_cts = '2' 
            THEN vcdes.monto_a_pagar 
            ELSE '0' 
        ...
        FROM
          ...
          LEFT JOIN 
            ...
            FROM
              permiso_personal 
            WHERE tip_permiso = 'VC' 
              AND id_cp = '".$id_seg_quin."') AS vcdes 
            ON vcdes.id_trab = vc.id_trab 
          LEFT JOIN 
            (
      ...
      LEFT JOIN 
        ...
        ON vc2.id_trab = vc.id_trab 
    GROUP BY vc.id_trab) AS vac 
  ...
  LEFT JOIN 
    ...
    WHERE pp.tip_permiso = 'DM' 
      AND pp.id_cp = '".$id_seg_quin."' 
    GROUP BY pp.id_trab) AS dme 
...
while($res=mysql_fetch_array($sql)){    
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue("CU$fila", utf8_encode($res["total_despositobcp_quincenal"]));
 ...
 /*INICIO  -  TOTALIZADO SEGUNDA QUINCENA*/
 
//rellenar con contenido  

$sql=mysql_query("SELECT DISTINCT 
  SUM(tr.sueldo_trab) AS sueldo_trab,
  ROUND(SUM(tr.sueldo_trab)/6,2)  AS sueldo_div6,
  ROUND((SUM(tr.sueldo_trab)*9)/100,2) AS sueldo_9por,
 (SUM(tr.sueldo_trab) +
  ROUND(SUM(tr.sueldo_trab)/6,2) +
  ROUND((SUM(tr.sueldo_trab)*9)/100,2)) AS sum_sueldo,
  SUM(IF(tr.asig_trab = 0, '', asig_trab)) AS asig_trab,
  SUM(IFNULL(hl_ma.dias_lab_sh, '') ) AS horas_lactancia,
  SUM(
  (
    CASE
    WHEN tr.fec_sal_trab = '0000-00-00 00:00:00' 
  ...
  SUM(HOUR(het.cant_abono_horas_al25)) + SUM(MINUTE(het.cant_abono_horas_al25)) / 60  AS cant_hor_ext_25,
  SUM(HOUR(het.cant_abono_horas_al35)) + SUM(MINUTE(het.cant_abono_horas_al35)) / 60  AS cant_hor_ext_35,
  SUM(HOUR(het.cant_abono_horas_dom)) + SUM(MINUTE(het.cant_abono_horas_dom)) / 60  AS cant_hor_ext_dominical,
  SUM(HOUR(het.cant_abono_horas_fer)) + SUM(MINUTE(het.cant_abono_horas_fer)) / 60  AS cant_hor_ext_feriado,
  SUM(IFNULL(vac.dias, '')) AS cant_dias_vacaciones,
  SUM(IF(dme.dias > 0, dme.dias, '')) AS cant_dias_descanso_medico,
  SUM(IF(sub.dias > 0, sub.dias, '')) AS cant_dias_subsidio,
  SUM(IF(lco.dias > 0, lco.dias, '')) AS cant_dias_lic_con_goce_haber,
  SUM(IF(lsi.dias > 0, lsi.dias, '')) AS cant_dias_lic_sin_goce_haber,
  SUM(HOUR(het.cant_dscto_enhoras)) + SUM(MINUTE(het.cant_dscto_enhoras)) / 60  AS cant_horas_faltadas,
  SUM(het.cant_dscto_endias) AS cant_dias_falta,
  SUM(  
  CASE
    WHEN het.cant_dscto_endias = '1' 
    THEN ROUND(
  ...
  END
   ...
   AS dscto_fondopension,
  rp.monto_reg_pen,
  SUM(IFNULL(rqc.mon_quin, ''))
   AS dscto_rentaquinta,
  '' AS dscto_segurovida,
  '' AS dscto_basedestajo,
  SUM(IFNULL(ROUND(dj.mon_men, 2), 0.00)) AS dscto_judicial,
  SUM(IFNULL(ROUND(dp.monto, 2), 0.00)) AS dscto_prestamo,
  SUM(IFNULL(ROUND(did.monto, 2), 0.00)) AS dscto_insumodestajeros,
  SUM(IFNULL(ROUND(dv.monto, 2), 0.00)) AS dscto_varios,
  SUM(IFNULL(ROUND(dm.monto, 2), 0.00)) AS dscto_menu,
  SUM(IFNULL(ROUND(aa.monto, 2), 0.00)) AS dscto_anticipo,
  SUM(
  /*INICIO TOTAL DESCUENTOS */
  /*INI- fondo de pension*/
  ROUND(
...
FROM
  ...
  LEFT JOIN 
    ...
    FROM
      renta_quinta_categoria 
    WHERE est_reg = '1' 
      AND id_quin = '".$id_pri_quin."') AS rqc 
    ON rqc.id_trab = tr.id_trab 
  LEFT JOIN 
    (SELECT 
      ma.id_trab,
        ...
        FROM
          ...
          LEFT JOIN 
            ...
            FROM
              horas_extras_personal hep 
            WHERE hep.abonar = '1'
            AND hep.id_fec_abono='".$id_pri_quin."') AS hep 
            ON tr.id_trab = hep.id_trab 
            AND hep.fecha BETWEEN ch.desde 
            AND ch.hasta 
          LEFT JOIN 
  ...
  LEFT JOIN 
    ...
    ON dee.id_trab = tr.id_trab 
  LEFT JOIN 
    (SELECT DISTINCT vc.id_trab,
     (IFNULL(vc2.monto,0) + IFNULL(vc1.monto,0)) AS monto,
      (IFNULL(vc2.monto_a_pagar,0) + IFNULL(vc1.monto_a_pagar,0)) AS monto_a_pagar,
       (IFNULL( vc2.dias,0) + IFNULL(vc1.dias,0)) AS dias,
     CONCAT(IFNULL(vc2.fechas,''),  ' ',IFNULL(vc1.fechas,'') )AS fechas
    FROM vacaciones  vc
    LEFT JOIN trabajador tr 
        ON tr.id_trab = vc.id_trab 
    LEFT JOIN cronograma_pagos cp 
    ...
    SELECT 
      vc.id_trab,
      IF(vc.id_cp = '".$id_pri_quin."', 'SI', '0.00') AS monto,
      CASE
        WHEN tr.id_pag_vac_cts = '1' 
        THEN ROUND(
          (tr.sueldo_trab / 30) * pd.dias_vc + 0.0000000001,
    ...
    FROM
      ...
      LEFT JOIN 
        ...
        FROM
          permiso_personal 
        WHERE tip_permiso = 'VC' 
          AND id_cp = '".$id_pri_quin."') AS vcdes 
        ON vcdes.id_trab = vc.id_trab 
      LEFT JOIN 
        (SELECT 
          vc.id_trab,
    ...
    WHERE vc.fec_del BETWEEN cp.desde 
      AND cp.hasta 
      ) AS  vc1 ON vc1.id_trab= vc.id_trab
   LEFT JOIN 
   (
    SELECT 
      vc.id_trab,
    ...
    WHERE vc.fec_al BETWEEN cp.desde 
      ...
      AND cp.hasta
     GROUP BY vc.id_trab
     ) AS vc2 ON vc2.id_trab= vc.id_trab
    GROUP BY vc.id_trab )
     AS vac 
    ON vac.id_trab = tr.id_trab 
  LEFT JOIN 
...
$sql_pla=mysql_query("SELECT DISTINCT 
  SUM(tr.sueldo_trab) AS sueldo_trab,
  ROUND(SUM(tr.sueldo_trab)/6,2)  AS sueldo_div6,
  ROUND((SUM(tr.sueldo_trab)*9)/100,2) AS sueldo_9por,
 (SUM(tr.sueldo_trab) +
...
FROM
  ...
  LEFT JOIN 
    (SELECT 
      aee.id_trab,
      CASE
        WHEN aee.fec_des1 = '27' 
        THEN mon_des1 
        WHEN aee.fec_des2 = '27' 
        THEN mon_des2 
        WHEN aee.fec_des3 = '27' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
...
while($res_pla=mysql_fetch_array($sql_pla)){     
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue("CS$fila", utf8_encode($res["abono_regularizacion"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("CT$fila", utf8_encode($res["otros_exceso_dscto_quincenal"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("CU$fila", utf8_encode($res_pla["total_despositobcp_quincenal"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("CV$fila", utf8_encode($res["bono_quincenal"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("CW$fila", utf8_encode($res["bono_destajo_quincenal"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("CX$fila", utf8_encode($res["vacaciones_compradas_otros"]));
  $objPHPExcel->getActiveSheet()->SetCellValue("CY$fila", utf8_encode($res["total_hextras"]));
  ...
  //Establecer estilo 
  //coloca el texto en amarillo y negrita
  $objPHPExcel->getActiveSheet()->setSharedStyle($regularizaciones, "AJ$fila:AN$fila");
  $objPHPExcel->getActiveSheet()->setSharedStyle($regularizaciones, "AS$fila:DA$fila");
  ...
  $objPHPExcel->getActiveSheet()->SetCellValue("AJ$fila", utf8_encode($res["sueldo_div6"]));
  $objPHPExcel->getActiveSheet()->setSharedStyle($subtitulo, "AJ$fila");

  $fila+=1;
  $objPHPExcel->getActiveSheet()->SetCellValue("AJ$fila", utf8_encode($res["sueldo_9por"]));
  $objPHPExcel->getActiveSheet()->setSharedStyle($subtitulo, "AJ$fila");


  $fila+=1;
  $objPHPExcel->getActiveSheet()->SetCellValue("AJ$fila", utf8_encode($res["sum_sueldo"]));
  $objPHPExcel->getActiveSheet()->setSharedStyle($subtitulo, "AJ$fila");
...
/*FIN - TOTALIZADO SEGUNDA QUINCENA*/
  ...
  $objPHPExcel->getActiveSheet()->getColumnDimension('CM')->setWidth(12);
  $objPHPExcel->getActiveSheet()->getColumnDimension('CN')->setWidth(10);
  $objPHPExcel->getActiveSheet()->getColumnDimension('CO')->setWidth(10);
  $objPHPExcel->getActiveSheet()->getColumnDimension('CP')->setWidth(12);
  $objPHPExcel->getActiveSheet()->getColumnDimension('CQ')->setWidth(13);
  ...
  $objPHPExcel->getActiveSheet()->getColumnDimension('CZ')->setWidth(12);
...
LEFT JOIN tabla_maestra_detalle AS tfop ON
  ...
  LEFT JOIN 
    ...
    FROM
      horas_extras_personal hep
    WHERE hep.id_fec_abono='".$id_pri_quin."'
    AND hep.abonar='1') AS hep 
    ON tr.id_trab = hep.id_trab 
    AND hep.fecha BETWEEN ch.desde 
    AND ch.hasta 
...
FROM
  ...
  LEFT JOIN pago_destajeros AS pd 
    ON pd.id_trab = tr.id_trab 
    AND pd.id_pd = '".$id_pri_quin."' 
  LEFT JOIN abono_regularizacion AS ar 
    ON ar.id_trab = tr.id_trab 
    AND ar.fec_abo_reg = '".$id_pri_quin."' 
  LEFT JOIN descuentos_judiciales AS dj 
    ON dj.id_trab = tr.id_trab 
    AND dj.est_des_jud = '1' 
  LEFT JOIN 
    (SELECT 
      pre.solicitante AS id_trab,
      CASE
        WHEN pre.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN pre.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN pre.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        WHEN pre.fec_des4 = '".$id_pri_quin."' 
        THEN mon_des4 
        WHEN pre.fec_des5 = '".$id_pri_quin."' 
        THEN mon_des5 
        WHEN pre.fec_des6 = '".$id_pri_quin."' 
        THEN mon_des6 
        WHEN pre.fec_des7 = '".$id_pri_quin."' 
        THEN mon_des7 
        WHEN pre.fec_des8 = '".$id_pri_quin."' 
        THEN mon_des8 
        WHEN pre.fec_des9 = '".$id_pri_quin."' 
        THEN mon_des9 
        WHEN pre.fec_des10 = '".$id_pri_quin."' 
        THEN mon_des10 
        WHEN pre.fec_des11 = '".$id_pri_quin."' 
        THEN mon_des11 
        WHEN pre.fec_des12 = '".$id_pri_quin."' 
        THEN mon_des12 
        WHEN pre.fec_des13 = '".$id_pri_quin."' 
        THEN mon_des13 
        WHEN pre.fec_des14 = '".$id_pri_quin."' 
        THEN mon_des14 
        WHEN pre.fec_des15 = '".$id_pri_quin."' 
        THEN mon_des15 
        WHEN pre.fec_des16 = '".$id_pri_quin."' 
        THEN mon_des16 
        WHEN pre.fec_des17 = '".$id_pri_quin."' 
        THEN mon_des17 
        WHEN pre.fec_des18 = '".$id_pri_quin."' 
        THEN mon_des18 
        WHEN pre.fec_des19 = '".$id_pri_quin."' 
        THEN mon_des19 
        WHEN pre.fec_des20 = '".$id_pri_quin."' 
        THEN mon_des20 
        WHEN pre.fec_des21 = '".$id_pri_quin."' 
        THEN mon_des21 
        WHEN pre.fec_des22 = '".$id_pri_quin."' 
        THEN mon_des22 
        WHEN pre.fec_des23 = '".$id_pri_quin."' 
        THEN mon_des23 
        WHEN pre.fec_des24 = '".$id_pri_quin."' 
        THEN mon_des24 
        WHEN pre.fec_des25 = '".$id_pri_quin."' 
        THEN mon_des25 
        WHEN pre.fec_des26 = '".$id_pri_quin."' 
        THEN mon_des26 
        WHEN pre.fec_des27 = '".$id_pri_quin."' 
        THEN mon_des27 
        WHEN pre.fec_des28 = '".$id_pri_quin."' 
        THEN mon_des28 
        WHEN pre.fec_des29 = '".$id_pri_quin."' 
        THEN mon_des29 
        WHEN pre.fec_des30 = '".$id_pri_quin."' 
        THEN mon_des30 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      did.id_trab,
      CASE
        WHEN did.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN did.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN did.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      dv.id_trab,
      CASE
        WHEN dv.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN dv.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN dv.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      dm.id_trab,
      CASE
        WHEN dm.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN dm.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN dm.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      aa.id_trab,
      CASE
        WHEN aa.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN aa.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN aa.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      dee.id_trab,
      CASE
        WHEN dee.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN dee.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN dee.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
  ...
  LEFT JOIN 
    (SELECT 
      aee.id_trab,
      CASE
        WHEN aee.fec_des1 = '".$id_pri_quin."' 
        THEN mon_des1 
        WHEN aee.fec_des2 = '".$id_pri_quin."' 
        THEN mon_des2 
        WHEN aee.fec_des3 = '".$id_pri_quin."' 
        THEN mon_des3 
        ELSE '0.00' 
      END AS monto 
    FROM
...
while($res=mysql_fetch_array($sql)){    
 ...
 /*INICIO  -  TOTALIZADO PRIMERA QUINCENA*/
...
/*FIN - TOTALIZADO PRIMERA QUINCENA*/

/*INICIO - TOTALIZADO PRIMERA QUINCENA SOLO PLANILLA*/



/*FIN - TOTALIZADO PRIMERA QUINCENA SOLO PLANILLA*/
