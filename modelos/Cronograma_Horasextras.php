...
Class Cronograma_Horasextras
{
	...
	public function insertar($id_ano, $des_fec_pag, $fec_pag)
	{
		...
		{
			$sql_detalle = "INSERT INTO cronograma_horasextras(id_ano, des_fec_pag, fec_pag) 
	...
	public function editar($id_ano, $des_fec_pag, $fec_pag, $desde, $hasta  )
	{
		...
		while ($num_elementos < count($des_fec_pag))
		{			
			$sql_detalle="UPDATE cronograma_horasextras SET    fec_pag='$fec_pag[$num_elementos]',  desde='$desde[$num_elementos]',   hasta='$hasta[$num_elementos]'     WHERE id_ano='$id_ano' AND des_fec_pag='$des_fec_pag[$num_elementos]'  ";
			ejecutarConsulta($sql_detalle) or $sw = false;
			$num_elementos=$num_elementos + 1;
		}
	...
	{
		$sql="SELECT DISTINCT 'CP' AS cp, obs,   id_ano, TbPea.Des_Corta AS Ano, cp.est_reg FROM cronograma_horasextras cp
				LEFT  JOIN 	tabla_maestra_detalle TbPea ON
				TbPea.cod_argumento=  cp.id_ano
				AND TbPea.Cod_tabla='TPEA'
