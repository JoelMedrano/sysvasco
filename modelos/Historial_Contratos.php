...
Class Historial_Contratos
{
	...
	{
		$sql="SELECT    '-' AS correlativo,
		 				Tra.id_trab,
				       Tra.num_doc_trab AS nro_doc,
				       CONCAT(Tra.apepat_trab, ' ' , Tra.apemat_trab, ' ', Tra.nom_trab)   AS apellidosynombres ,
				       TbAre.Des_Larga AS area_trab,
				       TbSua.des_larga AS sucursal, 
				       DATE_FORMAT(con.fec_ini_con, '%d/%m/%Y') AS fec_ini_con,  
				       DATE_FORMAT(con.fec_fin_con, '%d/%m/%Y') AS fec_fin_con,
				       CONCAT(con.tie_ren_con, ' MESES') AS tie_ren_ant,   
				       TbSic.des_larga AS situacion_informativa_actual
								FROM contratos con 
