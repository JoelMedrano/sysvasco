...
Class Descuentos_En_Efectivo
{
	...
	public function insertar(                       $id_trab,	
													...
													$pc_reg)
	{
		$sql="INSERT INTO descuentos_en_efectivo (id_trab,
												 fec_suc,
												 detalle,
												 num_cuotas,
												 cantidad,
	...
	public function editar(                         $id_des_efe,
													$id_trab,	
													$fec_suc,
													$detalle,
													$num_cuotas,
													...
													$pc_reg)
	{
		$sql="UPDATE descuentos_en_efectivo SET    id_trab='$id_trab',
												   fec_suc='$fec_suc',
			 									   detalle='$detalle',
												   ...
												   usu_mod='$usu_reg',
												   pc_mod='$pc_reg'
										     WHERE id_des_efe='$id_des_efe'";
		return ejecutarConsulta($sql);
	}

	//Implementamos un método para desactivar registros
	public function desactivar(                   $id_des_efe,
												  $fec_reg,
												  $usu_reg,
												  $pc_reg)
	{
		$sql="UPDATE descuentos_en_efectivo SET est_reg='0',
													   fec_anu='$fec_reg',
													   usu_anu='$usu_reg',
													   pc_anu='$pc_reg'
									     WHERE id_des_efe='$id_des_efe'";
	...
	public function activar(                      $id_des_efe,
		                                          $fec_reg,
												  $usu_reg,
												  $pc_reg)
	{
		$sql="UPDATE descuentos_en_efectivo SET   est_reg='1',
					 								     fec_act='$fec_reg',
					 								     usu_act='$usu_reg',
					 								     pc_act='$pc_reg'
			 							 WHERE id_des_efe='$id_des_efe'";
	...
	public function mostrar($id_des_efe)
	{
		$sql="SELECT     dee.id_des_efe,
				         ...
				         dee.tip_dscto,
				         dee.est_reg
				         FROM descuentos_en_efectivo dee
				INNER JOIN trabajador  tra ON
				dee.id_trab= tra.id_trab
				LEFT JOIN 
				(SELECT  dee.id_des_efe,  CONCAT (TbPea.Des_Corta,' - ',TbFpa1.des_larga ) AS   fecha1
				 FROM descuentos_en_efectivo dee 
				...
				)  AS TbFpa1 ON  TbFpa1.id_des_efe= dee.id_des_efe
				LEFT JOIN 
				(SELECT  dee.id_des_efe,  CONCAT (TbPea.Des_Corta,' - ',TbFpa2.des_larga ) AS   fecha2
				 FROM descuentos_en_efectivo dee 
				  ...
				  LEFT JOIN tabla_maestra_detalle AS TbPea ON TbPea.cod_argumento=  cp.id_ano AND TbPea.Cod_tabla='TPEA'  
				)  AS TbFpa2 ON  TbFpa2.id_des_efe= dee.id_des_efe
				LEFT JOIN 
				(SELECT  dee.id_des_efe,  CONCAT (TbPea.Des_Corta,' - ',TbFpa3.des_larga ) AS   fecha3
				 FROM descuentos_en_efectivo dee 
				  ...
				  LEFT JOIN tabla_maestra_detalle AS TbPea ON TbPea.cod_argumento=  cp.id_ano AND TbPea.Cod_tabla='TPEA'  
				)  AS TbFpa3 ON  TbFpa3.id_des_efe= dee.id_des_efe
				WHERE  dee.id_des_efe='$id_des_efe'";
