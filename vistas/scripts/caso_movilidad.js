...
function listar()
{
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/caso_movilidad.php?op=listar',
					type : "get",
					dataType : "json",
					error: function(e){
...
{ 
	...
	$.ajax({
		url: "../ajax/caso_movilidad.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(id_caso_mov)
{
	$.post("../ajax/caso_movilidad.php?op=mostrar",{id_caso_mov : id_caso_mov}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);

		
		$("#id_caso_mov").val(data.id_caso_mov);
...
function desactivar(id_abo_reg)
{
	bootbox.confirm("¿Está seguro de desactivar?", function(result){
		if(result)
        {
        	$.post("../ajax/caso_movilidad.php?op=desactivar", {id_abo_reg : id_abo_reg}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
...
function activar(id_abo_reg)
{
	bootbox.confirm("¿Está seguro de activar?", function(result){
		if(result)
        {
        	$.post("../ajax/caso_movilidad.php?op=activar", {id_abo_reg : id_abo_reg}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
