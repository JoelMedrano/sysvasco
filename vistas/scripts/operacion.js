...
{
	$.post("../ajax/operacion.php?op=mostrar",{id_operacion : id_operacion}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);

		$("#nombre").val(data.operacion);
...
function desactivar(id_operacion)
{
	bootbox.confirm("¿Está Seguro de desactivar la Operacion?", function(result){
		if(result)
        {
        	$.post("../ajax/operacion.php?op=desactivar", {id_operacion : id_operacion}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function activar(id_operacion)
{
	bootbox.confirm("¿Está Seguro de activar la Operacion?", function(result){
		if(result)
        {
        	$.post("../ajax/operacion.php?op=activar", {id_operacion : id_operacion}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
