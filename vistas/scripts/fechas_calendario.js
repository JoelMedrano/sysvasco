...
function listarArticulos() {
	tabla = $('#tblarticulos').dataTable({
		...
		"ajax": {
			url: '../ajax/fechas_calendario.php?op=selectHorasExtrasNoAbonadas',
			type: "get",
			dataType: "json",
			error: function (e) {
...
function guardaryeditar(e) {
	...
	$.ajax({
		url: "../ajax/fechas_calendario.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function listar() {
	tabla = $('#tbllistado').dataTable({
		...
		"ajax": {
			url: '../ajax/fechas_calendario.php?op=listar',
			type: "get",
			dataType: "json",
			error: function (e) {
				console.log(e.responseText);
...
function mostrar(id_ano) {
	...
	$.post("../ajax/fechas_calendario.php?op=mostrar",{id_ano : id_ano}, function(data, status)
	{
	...
	$.post("../ajax/fechas_calendario.php?op=listarDetalle&id=" + id_ano, function (r) {
		$("#detalles").html(r);
...
function desabilitar_abono(id_cp) {
	bootbox.confirm("¿Está Seguro de desabilitar abono?", function (result) {
		if (result) {
			$.post("../ajax/fechas_calendario.php?op=desabilitar_abono", {
				id_cp: id_cp
			}, function (e) {
				bootbox.alert(e);
				tabla.ajax.reload();
...
function habilitar_abono(id_cp) {
	bootbox.confirm("¿Está Seguro de habilitar abono?", function (result) {
		if (result) {
			$.post("../ajax/fechas_calendario.php?op=habilitar_abono", {
				id_cp: id_cp
			}, function (e) {
				bootbox.alert(e);
				tabla.ajax.reload();
