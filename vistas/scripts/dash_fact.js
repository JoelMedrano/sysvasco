...
function listar() {
	...
	tabla = $('#tbllistado').dataTable({
		...
		"ajax": {
			url: '../ajax/facturacion.php?op=listarDoc',
			data: {
...
function mostrar(num_mov) {
	$.post("../ajax/facturacion.php?op=mostrar", {
		num_mov: num_mov
	...
	$.post("../ajax/facturacion.php?op=listarDetalle&id=" + num_mov, function (r) {
		$("#detalles").html(r);
	});
}
...
function aprobar(num_mov,cod_alm) {
	bootbox.confirm("¿Está Seguro de aprobar el documento?", function (result) {
		if (result) {
			$.post("../ajax/facturacion.php?op=aprobar", {
				num_mov: num_mov,
				cod_alm: cod_alm
			}, function (e) {
				bootbox.alert(e);
...
function rechazar(num_mov,cod_alm) {
	bootbox.confirm("¿Está Seguro de rechazar el documento?", function (result) {
		if (result) {
			$.post("../ajax/facturacion.php?op=rechazar", {
				num_mov: num_mov,
				cod_alm: cod_alm
			}, function (e) {
				bootbox.alert(e);
