...
function init(){
	...
	$.post("../ajax/descuentos_en_efectivo.php?op=selectTrabajadoresActivos", function(r){
	            $("#id_trab").html(r);
	...
	$.post("../ajax/descuentos_en_efectivo.php?op=selectTipoDsctoPrestamo", function(r){
	            $("#tip_dscto").html(r);
	...
	//Cargamos los items al select Tipo de Descuento (Por planilla o interno)
	$.post("../ajax/descuentos_en_efectivo.php?op=selectModalidadPrestamo", function(r){
	            $("#modalidad").html(r);
...
function listar()
{
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/descuentos_en_efectivo.php?op=listar',
					type : "get",
					dataType : "json",
					error: function(e){
...
{ 
	...
	$.ajax({
		url: "../ajax/descuentos_en_efectivo.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
function mostrar(id_des_efe)
{
	$.post("../ajax/descuentos_en_efectivo.php?op=mostrar",{id_des_efe : id_des_efe}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);

		$("#id_des_efe").val(data.id_des_efe);
...
function desactivar(id_des_efe)
{
	bootbox.confirm("¿Está Seguro de desactivar el descuento?", function(result){
		if(result)
        {
        	$.post("../ajax/descuentos_en_efectivo.php?op=desactivar", {id_des_efe : id_des_efe}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
...
function activar(id_des_efe)
{
	bootbox.confirm("¿Está Seguro de activar el descuento?", function(result){
		if(result)
        {
        	$.post("../ajax/descuentos_en_efectivo.php?op=activar", {id_des_efe : id_des_efe}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
